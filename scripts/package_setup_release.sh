...
#
# Writes: dist/final-fantasy-ix-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix final-fantasy-ix \
  --exe-name Final_Fantasy_IX_Recompiled \
  --display-name "Final Fantasy IX Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --runtime-dir mods \
  --disc-hint "Final Fantasy IX (USA, Canada) (Disc 1) (Rev 1).cue" \
